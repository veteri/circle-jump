...
Player.prototype = {
    ...
    update: function(delta, map) {
        ...
        switch (this.physicMode) {
            case 1: this.gravityScale = 1; break;
            case 2: this.gravityScale = 0.95; break;
            case 3: this.gravityScale = 0.85; break;
        }
        ...
        this.vx = this.constrain(this.vx + this.cycleVX, -this.maxvx, this.maxvx);
        this.vy = this.constrain(this.vy + this.cycleVY, -this.maxvy, this.maxvy);

        //Stutter fix
        if (((this.vx > 0) && wasGoingLeft)
            || ((this.vx < 0) && wasGoingRight)) {
    ...
    draw: function (context, camera, frameCount, gameState) {
        ...
        if ((frameCount % this.animationDelay) === 0 || this.currentFrame === null) {
