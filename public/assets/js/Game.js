...
Game.prototype = {
	...
	bindEvents: function() {
		...
		document.addEventListener("keydown", function(event) {
			return self.player.handleButtonEvent.bind(
				self.player,
			)(event, event.keyCode, true);
		});
			...
			switch (event.keyCode) {
				case self.controls.restartLevel:
					if (
						...
						self.state === self.states.editorSandbox
					) {
						self.retry();
					}
					break;
				case self.controls.startLevel:
					if (
						...
						!self.player.levelComplete
					) {
						self.reset();
						self.start();
						UI.gameCanvas.displayEscapeNotification();
					} else {
						return self.player.handleButtonEvent.bind(self.player)(
							event,
							event.keyCode,
							false,
						);
	...
	changePerspective: function(perspective) {
		...
		this.camera.setDimensions(settingToApply.width, settingToApply.height);
	...
	loadAssets: function(assets, target = null, label = null) {
		...
		return new Promise(function(resolve, reject) {
			...
			const onImageLoad = function() {
				console.log(
					"%c[AssetLoader] Successfully loaded " + this.src,
					"color: green",
				);
				...
				if (--assetCount === 0) {
					console.log(
						"%c[AssetLoader] All " +
							totalAssetCount +
							" requested game assets have been loaded.",
						"color: green",
					);
				...
				let percent =
					((totalAssetCount - assetCount) / totalAssetCount) * 100;
				UIController.gameLoader.setProgress(percent);
			};

			const onImageError = function() {
				console.log(
					"%c[AssetLoader] Failed to load " + this.src,
					"color: red",
				);
				reject();
			};
			...
			const loadImageFromAssetObject = function(assetObject) {
				for (let key in assetObject) {
					if (
						assetObject.hasOwnProperty(key) &&
						typeof target[key] !== "object"
					) {
						target[key] = loadImage(assetObject[key]);
					} else {
						console.log(
							"%c[AssetLoader] Resource already loaded (" +
								assetObject[key] +
								")",
							"color: orange",
						);
			...
			if (Array.isArray(assets) && typeof assets[0] === "string") {
				...
				if (Array.isArray(target) && target.length > 0) {
					...
					console.log(
						"%c[AssetLoader] Resources already loaded for array target",
						"color: orange",
					);
					UIController.gameLoader.hide();
					resolve();
			...
			if (typeof assets === "object") {
				...
				if (Array.isArray(assets) && typeof assets[0] === "object") {
					...
					assets.forEach(assetObject =>
						loadImageFromAssetObject(assetObject),
					);
				} else {
	...
	resetPassedTime: function() {
		this.timePassedOffset =
			this.timePassed + (this.Utility.getTimestamp() - this.last) / 1000;
	},
		...
		return new Promise(function(resolve, reject) {
			...
			self.map
				...
				.then(function() {
					...
					console.log("bg1", mapBackgrounds);

					loadMapBackgrounds = self.map.backgroundScenes.map(
						background => {
					...
					Promise.all(loadMapBackgrounds).then(() => {
						//Prerender all map bgs to off canvases
						self.map.prerenderAll();
						resolve();
					});
				})
				.catch(function(result) {
					alert(
						"The map with the id '" + result + "' does not exist.",
					);
					self.start();
				});
		});
	...
	submitTime: function() {
		 ...
		 */
		const _0x9ef4 = [
			"\x67\x65\x74\x50\x61\x73\x73\x65\x64\x54\x69\x6D\x65",
			...
			"\x70\x61\x72\x73\x65",
			"\x72\x61\x6E\x6B\x69\x6E\x67\x73",
			"\x74\x69\x6D\x65",
			"\x6C\x6F\x67",
			"\x61\x64\x64\x45\x76\x65\x6E\x74\x4C\x69\x73\x74\x65\x6E\x65\x72",
			"\x50\x4F\x53\x54",
			"\x2F\x6D\x61\x70\x2F\x73\x75\x62\x6D\x69\x74\x2D\x74\x69\x6D\x65",
			"\x6F\x70\x65\x6E",
			...
			_0x31b6x5[_0x9ef4[14]](_0x9ef4[6], function() {
				if (this[_0x9ef4[7]] === 0x8000 >> 13) {
					if (this[_0x9ef4[8]] === (0xff & 0x80) + 0x48) {
						...
						_0x31b6x3({
							rankings: _0x31b6xc[_0x9ef4[11]],
							time: _0x31b6xc[_0x9ef4[12]],
						});
					} else {
						_0x31b6x4();
					}
	...
	complete: function() {
		...
		this.submitTime().then(function(response) {
	...
	showStartScreen: function() {
		...
		console.log(
			`[start screen end] tp: ${this.timePassed}, tpo: ${this.timePassedOffset}`,
		);
	},
	...
	init: function() {
		...
		return new Promise(function(resolve, reject) {
			...
			Promise.all([
				...
				.then(function() {
					//Prerender all player sprites
					self.player.prerenderAll();
					//Prerender all tile sprites
					Tile.prerenderAll();
					console.log(Tile.prototype.sprites);
					console.log(self.map.backgrounds);
